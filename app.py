...
def evaluate_pro(dx, reasoning, plan, case, profession, confidence, selected_ddx):
    ...
    # ดึงคำตอบที่เหมาะสมกับวิชาชีพนั้นๆ จาก JSON (ถ้าไม่มีจะใช้คำตอบกลาง)
    ...
    # 1. Dx Accuracy (5 pts) - AI Semantic Check
    try:
    ...
    # เพิ่มคะแนนถ้ามี Keyword ที่ตรงกับบทบาทวิชาชีพ
    role_keywords = {
    ...
    # Bonus for role-specific terminology
...
def load_cases():
    try:
        ...
        return [{"block":"General", "difficulty":"easy", "scenario":{"en":"Standard Case."}, "answer":"Sample Dx"}]
...
with st.sidebar:
    ...
    st.divider()
    # Updated Multi-Disciplinary Options
...
if page == "📖 Comprehensive User Guide":
    ...
    st.write("""
    The **Advanced Clinical Reasoning (ACLR) Loop** is a cognitive simulation designed to bridge the gap between 
    theoretical knowledge and bedside decision-making. Unlike Multiple Choice Questions (MCQs), 
    ACLR forces you to **formulate** your own thoughts and **calibrate** your confidence.
    """)

    st.markdown("### 👥 Interprofessional Perspectives")
    st.write("Each role has a different 'Primary Focus' in this simulator:")
    guide_cols = st.columns(3)
    guide_cols[0].info("**Clinicians (Dr/Dent/Vet):** Focus on Definitive Diagnosis and Intervention.")
    guide_cols[1].success("**Pharmacy:** Focus on Pharmacotherapy, Drug Safety, and Interactions.")
    guide_cols[2].warning("**Nursing/AMS/PH:** Focus on Monitoring, Diagnostics, and Prevention.")
    ...
    rubric = {
        ...
        "Criteria": [
            "Semantic match with the professional-specific target diagnosis.",
            "Use of evidence-based keywords and role-specific terminology.",
            "Correct selection of immediate Next Step and Patient Disposition.",
            "Mandatory exclusion of Red Flags and avoiding dangerous discharge."
        ]
    }
    st.table(pd.DataFrame(rubric))

    st.markdown("### 🛠 Step-by-Step Instruction")
    st.info("""
    1. **Gather Data:** Review the 'Scenario' and 'Diagnostics' tabs.
    2. **Exclude Red Flags:** Use the 'Must-Exclude' box to select life-threatening conditions.
    3. **Formulate Dx:** Type your diagnosis based on your professional role.
    4. **Justify:** Write the 'Reasoning'. Use logical connectors (e.g., 'Due to X, Y occurred').
    5. **Act:** Choose the most appropriate Next Step and Disposition.
    6. **Debrief:** Submit to compare with other professionals' perspectives.
    """)
...
st.markdown("---")
st.caption("ACLR Professional v5.0 Multi-Disciplinary | Clinical Decision Simulator 2026")
